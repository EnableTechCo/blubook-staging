...
| Public signup | `/signup`, `ClientSignUpWizard`, `createClientAccount` | Client creates its login, business, package, intake, compliance checklist, and initial requests in one guided flow | Login links directly to Client account creation | Functionally complete | Preserve self-service ownership and keep Provider/Staff provisioning internal | Public account creation still depends on the active catalogue and configured Supabase service role | Client self-service signup PR |
...
| Staff onboarding review | `/dashboard/onboardings`, compliance review actions | Staff review the compliance cases created by Clients; Staff no longer create Client accounts | Staff Client Onboarding export | Functionally complete, visual alignment needed | Preserve the review queue and keep account creation on the Client-facing signup flow | No lead/needs/agreement/draft activation stages or coverage confirmation | Client self-service signup PR |
