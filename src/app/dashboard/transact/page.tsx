...
export default async function TransactPage({
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      <ul className="grid gap-px border border-ink/30 bg-ink/30 sm:grid-cols-2 lg:grid-cols-3">
        {TRANSACTIONS.map((item) => {
          const inner = (
            <>
              ...
              <span className="mt-6 block border-t border-ink/30 pt-3 text-[9px] uppercase tracking-[0.14em] text-ink/50">
                {item.destination}
              </span>
            </>
          );
