...
export function RoleLoginNav({ activeRole }: { activeRole?: LoginRole }) {
  return (
    <nav aria-label="Choose a sign-in context" className="mt-8">
        ...
        {links.map(({ role, label }) => {
          ...
          return (
            <Link
              ...
              className={[
                "flex min-h-11 items-center justify-center border-r border-ink px-2 font-body text-xs font-semibold transition-colors last:border-r-0",
                "focus-visible:relative focus-visible:z-10",
                active
                  ? "bg-ink text-paper-light"
                  : "bg-paper-light text-ink hover:bg-cobalt-wash hover:text-cobalt-deep",
              ].join(" ")}
            >
              {label}
            </Link>
