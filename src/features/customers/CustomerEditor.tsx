...
import { updateCustomerDetails, type UpdateCustomerState } from "@/features/customers/actions";
...
function Section({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return (
    <fieldset className="grid gap-5 border-t border-ink py-7 lg:grid-cols-[13rem_minmax(0,1fr)] lg:gap-10">
      ...
      <div><h2 className="font-heading text-2xl">{title}</h2><p className="mt-2 text-xs leading-5 text-ink/55">{description}</p></div>
...
export function CustomerEditor({ customer }: { customer: EditableCustomer }) {
  const [state, action, pending] = useActionState<UpdateCustomerState, FormData>(updateCustomerDetails, undefined);
  ...
  return (
    <form action={action} aria-busy={pending}>
      ...
      <Section title="Business details" description="Legal identity and the trading name shown throughout BluBook.">
      ...
      <Section title="Billing contact" description="The person who receives billing correspondence.">
      ...
      <Section title="Business address" description="The organisation's physical business address.">
      ...
      <Section title="Billing address" description="The address used for invoices and billing records.">
      ...
      <Section title="Tax information" description="The customer's South African VAT status.">
      ...
      <div className="flex justify-end border-t border-ink py-6"><Button type="submit" disabled={pending}>{pending ? "Saving…" : "Save customer details"}</Button></div>
    </form>
  );
}
