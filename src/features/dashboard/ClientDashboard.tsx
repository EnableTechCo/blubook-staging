...
export function ClientDashboard({ data }: { data: ClientDashboardData }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section aria-label="Account overview" className="grid gap-px bg-ink/30 sm:grid-cols-3">
      ...
      <Section title="Service arrangements" subtitle="The packages currently held by your business">
        {packages.length === 0 ? (
          ...
          <div className="grid gap-px border border-ink/30 bg-ink/30 lg:grid-cols-2">
            {packages.map((servicePackage) => (
              <article key={servicePackage.id} className="bg-paper p-5">
                ...
                <ul className="mt-6 border-t border-ink/30">
                  {servicePackage.client_package_line_items.map((lineItem, index) => (
                    <li
                      key={`${lineItem.name}-${index}`}
                      className="flex items-start justify-between gap-6 border-b border-ink/25 py-3 text-[13px]"
                    >
                ...
                {onboarding.onboarding_documents.length === 0 ? (
                  ...
                  <ul>
                    {onboarding.onboarding_documents.map((document) => (
                      <li
                        key={document.id}
                        className="grid gap-3 border-b border-ink/25 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
                      >
