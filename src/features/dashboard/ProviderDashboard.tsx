...
function ProviderStat({
  ...
  return (
    <div className="min-h-32 border border-ink/60 border-t-ink bg-paper-light/70 p-4 sm:p-5">
    ...
    <div className="space-y-8">
      ...
      <Section title="Pending offers" subtitle="Requests routed to you awaiting your response">
        {offers.length === 0 ? (
          ...
          <ul className="divide-y divide-ink/45 border-y border-ink">
      ...
      <Section title="Capabilities" subtitle="Services you are registered to deliver">
        {capabilities.length === 0 ? (
          ...
          <ul className="grid border-l border-t border-ink/60 sm:grid-cols-2 lg:grid-cols-3">
            {capabilities.map((capability, index) => (
              <li
                key={index}
                className={`flex min-h-20 items-center justify-between gap-4 border-b border-r border-ink/60 px-4 py-3 ${
