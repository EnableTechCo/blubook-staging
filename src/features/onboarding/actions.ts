...
export async function updateCustomerDetails(
  ...
  if (!staff || staff.user_type !== "staff") return { error: "Only staff can update clients." };
  ...
  const [profileUpdate, accountUpdate] = await Promise.all([
    admin
      .from("profiles")
  ...
  if (profileUpdate.error || accountUpdate.error) {
    return { error: profileUpdate.error?.message ?? accountUpdate.error?.message ?? "Update failed." };
  }

  revalidatePath("/dashboard", "layout");
  revalidatePath("/dashboard/onboardings");
  redirect(`/dashboard/onboardings?updated=${encodeURIComponent(input.tradingName)}`);
}
